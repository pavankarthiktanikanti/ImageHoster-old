...
public class ImageRepository {
    ...
    //The method creates an instance of EntityManager
    //Executes JPQL query to fetch the image from the database with corresponding image id
    //Returns the image in case the image is found in the database
    //Returns null if no image is found in the database
	//Changed the method to Pull the Image record using imageId for uniqueness of record
    public Image getImageById(Integer imageId) {
        EntityManager em = emf.createEntityManager();
        try {
            TypedQuery<Image> typedQuery = em.createQuery("SELECT i from Image i where i.id = :id", Image.class).setParameter("id", imageId);
